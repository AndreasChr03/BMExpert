...
if (isset($_POST['submit'])) {
    ...
    $property_id = isset($_POST['property_rented']) ? (int)$_POST['property_rented'] : null;
    ...
    if (empty($response['errors'])) {
        ...
        if ($stmt = mysqli_prepare($mysqli, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                $tenant_id = mysqli_insert_id($mysqli);
                $query="UPDATE property SET tenant_id = ?, status=? WHERE property_id = ?";
                $status='r';
                if ($stmt2 = mysqli_prepare($mysqli, $query)) {
                    mysqli_stmt_bind_param($stmt2, "isi", $tenant_id, $status, $property_id);
                    mysqli_stmt_execute($stmt2);
                    $mysqli->commit();

                    mysqli_stmt_close($stmt2);
                } else {
                    $response['errors'][] = "Error preparing the update statement for property table.";
                }
    ...
    if (!empty($response['success'])) {
        ...
        switch ($creator_role_id) {
            case 1:
                ...
                break;
            case 2:
                $redirectUrl = "../Views/users/dashboard_role/dashboard_owner/dashboard_owner_tenants.php";
                break;
        }
