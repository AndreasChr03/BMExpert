...
if (isset($_POST['addBill'])) {
    $buildingId = 1;  // Set directly to 1 as required
    ...
    $sql = "INSERT INTO bills (building_id, bill_title, cost, comment, bill, receipt) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("isdsss", $buildingId, $billTitle, $cost, $billComment, $billPhotoPath, $receiptPhotoPath);
        if ($stmt->execute()) {
...
if (isset($_POST['updateBill'])) {
    ...
    $stmt = $mysqli->prepare("UPDATE bills SET bill_title = ?, cost = ?, comment = ?, bill = ?, receipt = ? WHERE bill_id = ?");
    $stmt->bind_param("sdsssi", $billTitle, $cost, $billComment, $billPhotoPath, $receiptPhotoPath, $billId);
...
// Fetch bills for display only where building_id = 1
$sql = "SELECT bill_id, bill_title, cost, comment, bill, receipt, CREATED FROM bills WHERE building_id = 1 ORDER BY CREATED DESC";
...
<body>
    ...
    <div class="main">
        <div class="user-header d-flex justify-content-between align-items-center">
            ...
            <!-- Centered Button -->
            <button class="btn btn-primary" onclick="window.location.href='dashboard_admin_statistics.php';">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover" id="billTable">
            <thead>
                <tr>
                    <th class="center-text aa-column">A/A</th>
                    <th>Date</th>
                    <th>Title</th>
                    <th>Cost</th>
                    <th>Comment</th>
            ...
            <tbody>
                ...
                while ($row = $result->fetch_assoc()) :
                    ...
                    if (count($words) > 5) {
                        $comment = implode(' ', array_slice($words, 0, 17)) . '...';
                    } else {
                        $comment = $full_comment;
                    }
                ?>
                    <tr>
                        <td class="text-center aa-column"><?= $rowNumber++ ?></td>
                        <td><?= $formattedDate ?></td>
                        <td><?= htmlspecialchars($row['bill_title'] ?? '-') ?></td>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#addBillModal').on('hidden.bs.modal', function (e) {
                ...
                $(form).removeClass('was-validated'); // Remove the validation classes

                // Reset custom text inputs for file names
                $('#bill-file-name').val('No file chosen');
                $('#receipt-file-name').val('No file chosen');

                // Optionally reset any validation feedback visuals
